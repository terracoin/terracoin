...
## What about the terracoin.conf file?

If you are using a `masternode.conf` file you no longer need the `terracoin.conf` file. The exception is if you need custom settings (_thanks oblox_). In that case you **must** remove `masternode=1` from local `terracoin.conf` file. This option should be used only to start local Hot masternode now.
...
Search your Masternodes on https://terracoinninja.pl/masternodes.html
