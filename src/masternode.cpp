...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if(coins.vout[vin.prevout.n].nValue != MASTERNODE_COLLATERAL) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %s TRC, masternode=%s\n", MASTERNODE_COLLATERAL, vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 5000 TRC tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 5000 TRC tx -> 1 confirmation
